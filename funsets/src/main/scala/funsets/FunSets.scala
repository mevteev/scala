...
object FunSets {
  ...
  /*
  def forall_old(s: Set, p: Int => Boolean): Boolean = {
      var flag = false
  ...
  */
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int, flag : Boolean): Boolean = {
      if (a > bound) flag
      else if (contains(s, a) && !p(a)) false
      else iter(a + 1, flag || contains(s, a))
    }
    iter(-bound, false)
  }
   ...
   */
    ...
    /*def map_old(s: Set, f: Int => Int): Set = (x: Int) => {
      def iter(a : Int): Boolean = {
        if (a > bound) false
        else if ((f(a) == x) && contains(s, a)) true
        else iter(a + 1)
      }
