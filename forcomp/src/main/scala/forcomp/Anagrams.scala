...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    ((w.toLowerCase groupBy(e => e)) map( g => (g._1, g._2.length))).toList.sorted

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences =
    wordOccurrences(s.foldLeft("")((a,b) => b.concat(a)))
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] =
    loadDictionary groupBy wordOccurrences withDefaultValue(List())
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    ...
    def getList(c: (Char, Int)): List[Occurrences] =
      (for (n <- 0 to c._2) yield
        if (n == 0) List()
        else List((c._1, n))).toList

    occurrences match {
      case List() => List(List())
      case List(x) => getList(x)
      case x :: xs =>
        (for (
          c <- getList(x);
          cs <- combinations(xs)
